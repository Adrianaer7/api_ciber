...
const Formulario = ({producto, cargando}) => { //traigo producto de EditarProducto

    const navigate = useNavigate()

    //Validacion - Lee lo que voy escribiendo y lo valida
    const nuevoProductoSchema = Yup.object().shape({ //shape va a ser la forma que van a tener los datos cuando se crea un producto
        nombre: Yup.string()
    ...
    const handleSubmit = async valores => { //traigo los valores del onSubmit
        try {
            ...
            navigate("/")   //luego de enviar los datos, redirecciono al producto
    ...
    return (
            <div className='bg-white dark:bg-gray-900 mt-10 px-5 py-10 rounded-md shadow-md md:w-3/4 mx-auto'>
                <h1 className='text-gray-600 dark:text-white font-bold text-xl uppercase text-center'>{producto.nombre ? "Editar producto" : "Agregar producto"}</h1>

                <Formik 
                    initialValues={{    //tomo los valores de los input
                        nombre: producto?.nombre ?? "", //traigo el dato del producto si existe, sino, lo inicio como string vacio
                    ...
                    {({errors, touched}) => {   //en errors se almacena el nombre del campo y el msj. Touched es para que al salir de un input vacio, salga la alerta
                        return (
                            <Form className="mt-10">
                                ...
                                <input
                                    type="submit"
                                    value={producto.nombre ? "Editar producto" : "Agregar producto"}
...
Formulario.defaultProps = {
    producto: {}, //incia el formulario vacio a menos que me traiga un producto
    cargando: false //no muestra el spinner a menos que haya un producto para editar
}
